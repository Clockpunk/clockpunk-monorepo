...
import type { FeaturedArtworksSection as FeaturedArtworksSectionType } from '@/lib/sanity/types';
import { urlForImage } from '@/lib/sanity/image';

type Props = {
  section: FeaturedArtworksSectionType;
  id?: string;
  theme: 'dark' | 'light';
};

export function FeaturedArtworksSection({ section, id, theme }: Props) {
    ...
    <section id={id} className={`py-20 md:py-32 ${bgClass}`}>
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {items.slice(0, section.showCount).map((artwork) => (
            <Link
              key={artwork._id}
